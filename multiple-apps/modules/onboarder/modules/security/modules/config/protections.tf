...
resource "akamai_appsec_api_constraints_protection" "policy" {
  config_id          = akamai_appsec_configuration.config.config_id
...
resource "akamai_appsec_ip_geo_protection" "policy" {
  config_id          = akamai_appsec_configuration.config.config_id
  security_policy_id = akamai_appsec_api_constraints_protection.policy.security_policy_id
  enabled            = true
}
...
resource "akamai_appsec_slowpost_protection" "policy" {
  config_id          = akamai_appsec_configuration.config.config_id
  security_policy_id = akamai_appsec_reputation_protection.policy.security_policy_id
  enabled            = true
}
